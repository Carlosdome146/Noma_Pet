const DEMO_PRODUCTS=[
{id:'roller',name:'Rodillo reutilizable quitapelos',price:22.90,cat:'limpieza',emoji:'🧹',tag:'TOP',desc:'Recoge pelo de sofás, ropa y asientos sin recambios adhesivos.',variants:[]},
{id:'bottle',name:'Botella 3 en 1 de paseo',price:21.90,cat:'paseo',emoji:'💧',tag:'PASEO',desc:'Agua, bebedero y espacio auxiliar en un formato compacto.',variants:[]},
{id:'slow',name:'Comedero lento',price:18.90,cat:'hogar',emoji:'🥣',tag:'DIARIO',desc:'Diseño laberinto para alargar el tiempo de comida de forma sencilla.',variants:[]},
{id:'hammock',name:'Protector de asiento tipo hamaca',price:49.90,cat:'viaje',emoji:'🚗',tag:'VIAJE',desc:'Protección impermeable para el asiento trasero, fácil de colocar y limpiar.',variants:[]},
{id:'glove',name:'Guante de cepillado',price:14.90,cat:'limpieza',emoji:'🧤',tag:'CUIDADO',desc:'Cepillado cómodo para retirar pelo suelto durante el cuidado diario.',variants:[]},
{id:'bags',name:'Kit paseo bolsas + dispensador',price:12.90,cat:'paseo',emoji:'♻️',tag:'RECURRENTE',desc:'Un básico ligero y recurrente para los paseos de cada día.',variants:[]}
];
...
function add(id,qty=1,variantId=null){
  ...
  if(variantsOf(p).length){
    ...
    if(!v){location.href=`producto.html?id=${encodeURIComponent(id)}`;return}
    chosen=v.id;
...
function productCard(p){
  ...
  const action=hasVariants
    ? `<a class="btn primary" href="producto.html?id=${encodeURIComponent(p.id)}">Ver opciones</a>`
    : `<button class="btn primary" data-add="${escapeHtml(p.id)}" onclick="add('${String(p.id).replace(/'/g,"\\'")}')">Añadir</button>`;
  return `<article class="product" data-cat="${escapeHtml(p.cat)}"><a href="producto.html?id=${encodeURIComponent(p.id)}" class="photo"><span class="tag">${escapeHtml(p.tag||'')}</span>${productVisual(p)}</a><div class="product-body"><h3>${escapeHtml(p.name)}</h3><p>${escapeHtml(p.desc||'')}</p><div class="price"><div><strong>${productPriceHtml(p)}</strong><div class="meta">${hasVariants?`${variantsOf(p).length} opciones disponibles`:'IVA incluido'}</div></div>${action}</div></div></article>`
}
...
function renderDetail(){
  ...
  const p=PRODUCTS.find(x=>x.id===id)||PRODUCTS[0];if(!p){el.innerHTML='<p>Producto no encontrado.</p>';return}
  ...
  const logistics=selected?(selected.warehouse||selected.shippingDaysMin!=null?`<div id="detailVariantMeta" class="detail-logistics"></div>`:'') : '';
  el.innerHTML=`${visual}<div class="detail"><span class="eyebrow">Selección NÓMA PET</span><h1>${escapeHtml(p.name)}</h1><div class="pricebig" id="detailPrice">${money(selected?.price??p.price)}</div>${variantButtons(p,DETAIL_VARIANT_ID)}${logistics}<p class="lead">${escapeHtml(p.description||p.desc||'')}</p><div style="display:flex;gap:12px;align-items:center;margin:24px 0"><div class="qty"><button onclick="changeQty(-1)">−</button><span id="qty">1</span><button onclick="changeQty(1)">+</button></div><button id="detailAddBtn" class="btn primary" type="button">Añadir al carrito</button></div></div>`;
...
function renderCart(){
  ...
  sum.innerHTML=`<h3>Resumen</h3><div class="row"><span>Productos</span><b>${money(subtotal)}</b></div><div class="row"><span>Envío</span><span>Se calcula según la tienda</span></div><div class="row total"><span>Total productos</span><span>${money(subtotal)}</span></div><a class="btn primary" style="width:100%;margin-top:14px" href="checkout.html">Continuar al checkout</a>`
}
